...
for (dfsurv_id in dfsurv_ids) {
  ...
  dfsurvdat$response_wave_ID <- dfsurv_id
}
...
dem2 <- read_csv("respondent_demographics2.csv")
...
dfsurvdat$Education_lvl <- NA
dfsurvdat$Education_lvl[dfsurvdat$Education %in% c("Completed some high school","Middle School - Grades 4 - 8")] <- "<High school"
dfsurvdat$Education_lvl[dfsurvdat$Education %in% c("High school graduate","Completed some college, but no degree")] <- "High school"
dfsurvdat$Education_lvl[dfsurvdat$Education %in% c("Associate Degree","Other post high school vocational training","Completed some graduate, but no degree")] <- "College"
dfsurvdat$Education_lvl[dfsurvdat$Education %in% c("Doctorate degree","Masters degree")] <- "Postgraduate"
dfsurvdat$Education <- factor(dfsurvdat$Education_lvl, levels=c("<High school","High school","College","Postgraduate"))
...
dat <- dfsurvdat[,c(
  "Zip", "Age", "Gender", "HHI", "PID_presurvey", 
  "Ethnicity", "Hispanic", "Education", "Region",
  "rid", "quality", "comments", 
  "StartDate", "EndDate"
)]
...
##### EDUCATION ####
#####------------------------------------------------------#
dat$educ <- as.character(dfsurvdat$Education)
dat$educ <- ifelse(is.na(dat$educ), "N/A", dat$educ)
dat$educ <- factor(
      dat$educ,
      levels = c(
        "N/A", "<High school", "Postgraduate", "High school", "College" 
      )
)
table(dat$educ)
...
rm(w,h,w_x_h,failed1,failed2,isreal,isfake,agree_lvls,
   dem,dem1,dem2,dfsurvdat_id,id,id_locations,X)
